...
function loadSceneData(): void{
  ...
  for (var i: number = 0; i < sceneJsonData.length; i++){
    var temp: scenes.NormalScene;
    //temp = new scenes.NormalScene();
    //temp = sceneJsonData[i];
    // sceneLibrary[sceneJsonData[i].ID] = new scenes.NormalScene();
    // sceneLibrary[sceneJsonData[i].ID] = temp;
    console.log(temp);
  }
